        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow ">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 ">¿Estas seguro de habilitar el servicio?</h3>
...
@section('scripts')
     <script>  
        ...
        function getPagination(currentPage){ 
            ...
            fetch('{{$menu['baseURL']."/services/list?page="}}'+currentPage+'&s='+s+'&'+status, { 
                headers: { 
        ...
        function confirmRecover(){
            fetch('{{$menu['baseURL']."/services/recover/"}}'+currentService, { 
                headers: { 
                    "X-CSRF-Token": document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'Accept': 'application/json', 
                },  
        ...
        function setRows(data){ 
            ...
            if(data.length){
                data.forEach(service => { 
                    let rowHTML = '<tr class="border-b border-gray-200"><td scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap"><a href="{{$menu['baseURL'].$menu['route']['services']['edit']('')}}'+service.id+'"  class="font-bold text-[#526270]">'+service.name+'</a>'+(service.status == "0" ? '<small class="block text-red-600"><i>Eliminado</i></small>' : "")+'</td>'+
                    ...
                    '<td class="px-4 py-3 flex items-center justify-end">'+
                        '<div class="group relative">'+
                            '<button class="inline-flex items-center p-0.5 text-sm font-medium text-center text-[#526270] hover:text-gray-800 rounded-lg focus:outline-none" type="button">'+
                                ...
                                '</svg>'+
                            '</button>'+
                            '<div class="absolute left-[-173px] '+(service.status == 1 ? "top-[-40px]" : "top-[-6px]")+' group-hover:block hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow">'+
                                (service.status ? '<ul class="py-1 text-sm text-gray-700" aria-labelledby="apple-imac-27-dropdown-button">'+ 
                                    '<li>'+
                                ...
                                '</div>' : 
                                '<div class="py-1">'+
                                    '<button onclick="recoveryService(\''+service.id+'\')" class="w-full text-left py-2 px-4 text-sm text-gray-700 hover:bg-gray-100">Recuperar</button>'+
                                '</div>')+
                            '</div>'+
                        '</div>'+
                    '</td></tr>';
